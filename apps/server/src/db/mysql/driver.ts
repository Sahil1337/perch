...
import { type DatabaseSchema, type QueryError } from "@perch/protocol";
...
import { commandOf, toQueryError, toResultColumn, type MysqlError } from "./types.js";
...
export class MysqlDriver extends BaseDriver<mysql.Pool, ThreadId> {
  ...
  private poolOptions(database: string | undefined, connectionLimit: number): mysql.PoolOptions {
    return {
  ...
  protected async createPool(database: string | undefined, max: number): Promise<mysql.Pool> {
    return mysql.createPool(this.poolOptions(database, max));
  }

  protected async checkPool(pool: mysql.Pool): Promise<void> {
  ...
  protected async executeStatement(sql: string, ctx: StatementContext<ThreadId>): Promise<void> {
    ...
    try {
      ...
      if (options.readOnly)
        await conn.query("set session transaction_read_only = 0").catch(() => {});
      conn.release();
    }
  }
