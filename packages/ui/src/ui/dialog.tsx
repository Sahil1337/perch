...
export function DialogPopup({
  ...
  /**
   * `sm` is for a dialog that asks one question — a filename, a confirmation. The default width
   * and radius are sized for a panel of settings, and at that size a two-field form reads as a
      ...
      >
        <DialogPrimitive.Popup
          className={cn(
            ...
            size === "sm" &&
              "max-w-sm rounded-xl before:rounded-[calc(var(--radius-xl)-1px)]",
            bottomStickOnMobile &&
...
export function DialogTitle({
  ...
  /** Pairs with `<DialogPopup size="sm">`: a heading, not a banner. */
  size?: "default" | "sm";
}): React.ReactElement {
  return (
    <DialogPrimitive.Title
      className={cn(
        ...
        size === "sm" && "font-sans font-medium text-sm",
        className,
      )}
...
export function DialogDescription({
  ...
  /** Pairs with `<DialogPopup size="sm">`. */
  size?: "default" | "sm";
}): React.ReactElement {
  return (
